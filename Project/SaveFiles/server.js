...
app.post('/users/signminute', (req, res) => 
{
    const { key, docname } = req.body;
    ...
    .then(user =>
    {
        gfs.files.findOne({ filename: docname }, (err, fileInfo) =>
        {
            if(fileInfo)
            {
                ...
                res.render('home.ejs', { error_msg: 'File not found' });
        });       
    });
});
...
app.post('/users/deciphermemo', (req, res) => 
{
    let {docname, privatekey} = req.body;
...
// @route GET /users/memo
// @desc Renders Memo EJS
app.post('/users/memo', (req, res) =>
{
    let { usernamereceiver, subject, memorandum, privatekey } = req.body;
